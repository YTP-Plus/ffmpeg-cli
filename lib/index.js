...
const FFmpeg = {
	...
	path: global.__execpath,
	runSync: cmd => cp.execSync(`"${global.__execpath}" ${cmd}`, {
		encoding: 'utf8'
	}),
	forceDownload: () => {
		try {
...
if (!fs.existsSync(global.__execpath)) {
	console.log('FFmpeg not detected, attempting to download...');

	// Check for possible corruption
	try {
		if (!FFmpeg.forceDownload() || !fs.existsSync(global.__execpath)) throw null;
		FFmpeg.runSync('-version');
		console.log(`FFmpeg verified at ${global.__execpath}`);
	} catch (e) {
